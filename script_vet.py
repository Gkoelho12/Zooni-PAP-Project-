...
vet_files = ['dashboard_veterinario.html', 'agenda_veterinario.html', 'tickets_veterinario.html', 'pacientes_veterinario.html', 'chat_veterinario.html']
...
for f in vet_files:
    ...
    # If the file has a sidebar tag, replace it. Otherwise don't touch HTML (chat_veterinario might not have it).
    if '<aside class="sidebar">' in content:
        content = re.sub(r'<aside class="sidebar">.*?</aside>', specific_html, content, flags=re.DOTALL)
    
    # Handle CSS replacement
    # We will look for `.sidebar {` and inject our CSS template before it, then remove the old sidebar css
    # Since old CSS varies, we will just try to find the start and end of sidebar-related css
    # To be safe, we'll do it manually file by file to avoid breaking other styles.
